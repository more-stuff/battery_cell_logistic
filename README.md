...
- **Ese mismo interruptor bloquea la edición.** Con `sync_activo` encendido no se puede modificar ni borrar ninguna caja (`verificar_caja_editable` en `routers/admin/guards.py`): a partir del envío es SILENA quien gestiona modificaciones y bajas. Es un único flag a propósito — si el worker pudiera exportar mientras un admin edita, el CSV saldría con los datos viejos y la caja quedaría en `EXPORTADO` sin reexportarse nunca. Con un solo flag, worker y edición no coinciden en el tiempo.
